...
import { Package, Plus, Search, Calendar, Building, FileText, Truck } from 'lucide-react';
import { apiClient } from '../services/apiClient';
import { InboundRecord, Warehouse, Product } from '../types';
...
export function Inbound() {
  ...
  const loadInbounds = async () => {
    try {
      ...
      const response = await apiClient.get<{
        data: { inbounds: InboundRecord[]; total: number; pages?: number }
      }>('/inbounds?' + query.toString());

      setRecords(response.data.inbounds);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.product_id || !formData.warehouse_id || !formData.quantity || !formData.supplier_name) {
    ...
    try {
      ...
      // Post ke inbounds
      const inboundResponse = await apiClient.post('/inbounds', inboundPayload);

      // --- Payload untuk transactions (sesuaikan format API /transactions) ---
      ...
      toast.success(t('inbound.inbound_added'));
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          <DialogTrigger asChild>
            <Button>
              ...
              {t('inbound.add_inbound')}
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-4xl">
            <DialogHeader>
              <DialogTitle>{t('inbound.add_inbound')}</DialogTitle>
              <DialogDescription>
                Add a new inbound record to track incoming inventory
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4"> {/* Mengubah grid */}
              ...
              {/* Supplier Name */}
              <div className="space-y-2 col-span-2 sm:col-span-3 lg:col-span-3">
                <Label htmlFor="supplier_name">{t('inbound.supplier_name')}</Label>
                <Input
                  id="supplier_name"
              ...
              {/* Supplier Contact */}
              <div className="space-y-2 col-span-2 sm:col-span-3 lg:col-span-3">
                <Label htmlFor="supplier_contact">{t('inbound.supplier_contact')}</Label>
                <Input
                  id="supplier_contact"
              ...
              {/* Unit Cost */}
              <div className="space-y-2 col-span-2 sm:col-span-3 lg:col-span-3">
                <Label htmlFor="unit_cost">{t('inbound.unit_cost')}</Label>
                <Input
                  id="unit_cost"
                  type="number"
              ...
              {/* Received Date */}
              <div className="space-y-2 col-span-2 sm:col-span-3 lg:col-span-3">
                <Label htmlFor="received_date">{t('inbound.received_date')}</Label>
                <Input
                  id="received_date"
                  type="date"
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1">
          <div className="relative">
            ...
            <Input
              placeholder="Search by product, SKU, supplier, or reference..."
              value={searchQuery}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Inbound Records</CardTitle>
        </CardHeader>
        <CardContent>
          {records.length === 0 ? (
            <div className="text-center py-8">
              <Package className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
              <p className="text-muted-foreground">No inbound records found</p>
            </div>
          ) : (
                  ...
                  {records.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <div>
                          ...
                          {record.supplier_contact && (
                            <div className="text-sm text-muted-foreground">{record.supplier_contact}</div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div>
                          {record.unit_cost && (
                            <div className="text-sm">{formatCurrency(record.unit_cost)}/unit</div>
                          )}
                          {record.total_cost && (
                            <div className="font-medium">{formatCurrency(record.total_cost)}</div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm">{formatDate(record.received_date)}</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
